...
public class AppointmentController {
        ...
        public ResponseEntity<RecurringAppointmentResponse> createRecurring(
                ...
                var response = new RecurringAppointmentResponse(
                                ...
                                result.failedAppointments().stream()
                                                .map(f -> new RecurringAppointmentResponse.FailedSlot(f.date(),
                                                                f.reason()))
                                                .toList());
        ...
        public ResponseEntity<Appointment> addItem(
                        @PathVariable String id,
                        @RequestBody @Valid AddItemRequest request) {
                return ResponseEntity
                                .ok(addAppointmentItemUseCase.execute(id, request.productId(), request.quantity()));
        }

        @Operation(summary = "Remover Produto da Comanda", description = "Remove um item do agendamento e devolve ao estoque.")
        @DeleteMapping("/{id}/items/{productId}")
        public ResponseEntity<Appointment> removeItem(
                        @PathVariable String id,
